...
const filePath = path.join(__dirname, '..', 'utils', 'drawingSystem.ts');
...
// 1303行开始是 export function createLightsaberStrokeMesh
// 1791行是函数结束的 }
// 1792行是空行
// 1793行开始是下一个注释

// 获取替换前后的内容 (0-indexed)
const before = lines.slice(0, 1302).join('\n');  // 1-1302行
const after = lines.slice(1792).join('\n');       // 1793行及之后
...
console.log('光剑函数已替换为Billboard实现');
console.log('新文件行数:', result.split('\n').length);
