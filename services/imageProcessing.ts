...
export const processImage = (
  ...
  try {
    positions = new Float32Array(estimatedCount * 3);
    colors = new Float32Array(estimatedCount * 3);
    sizes = new Float32Array(estimatedCount);
  } catch (e) {
    // 内存分配失败，降级到较小的粒子数
    console.warn(`Memory allocation failed for ${estimatedCount} particles, falling back to 300k`);
    estimatedCount = 300000;
    positions = new Float32Array(estimatedCount * 3);
    colors = new Float32Array(estimatedCount * 3);
    sizes = new Float32Array(estimatedCount);
  }
  ...
  // 使用subarray避免创建副本，减少内存占用
  // 注意：subarray共享底层buffer，不会复制数据
  return {
    positions: new Float32Array(positions.buffer, 0, particleIndex * 3),
    colors: new Float32Array(colors.buffer, 0, particleIndex * 3),
    sizes: new Float32Array(sizes.buffer, 0, particleIndex),
    count: particleIndex,
    originalWidth,
    originalHeight,
    scaleFactor,
