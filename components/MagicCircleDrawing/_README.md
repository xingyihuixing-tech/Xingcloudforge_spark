...
| `DrawingControlPanel.tsx` | 右侧面板 | 绘图模式控制面板，包含可折叠法阵列表、对称模式按钮(径向/万花筒/星芒/球面)、分割数滑块、通用分形设置(层数/缩放/角度)、高级对称参数滑块 |
...
- **none**: 无对称
- **radial**: 径向旋转对称（支持分形）
- **kaleidoscope**: 万花筒（旋转+镜像，支持分形）
- **starburst**: 星芒（奇偶分割缩放+分形）
- **sphere**: 球面（2D映射到3D球体表面，支持分形）
