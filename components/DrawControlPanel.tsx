...
import { DrawSettings, DrawMode, PlanetSceneSettings, BrushSettings, DrawingLayer } from '../types';
...
const DrawControlPanel: React.FC<DrawControlPanelProps> = ({ settings, setSettings, planetSettings }) => {
    ...
    // Ensure we have at least one layer initialized
    useEffect(() => {
        if (!settings.layers || settings.layers.length === 0) {
            const defaultLayer: DrawingLayer = {
                ...
                name: '默认图层',
                visible: true,
                bindPlanetId: settings.bindPlanetId || planetSettings.planets[0]?.id || null,
                tilt: { x: 0, y: 0, z: 0 },
                scale: 1,
                ...
                opacity: 1,
                blending: 'additive', // Default to additive for "energy" look
    ...
    const activeLayer = settings.layers?.find(l => l.id === settings.activeLayerId);

    const updateActiveLayer = (updates: Partial<DrawingLayer>) => {
        if (!activeLayer) return;
        setSettings(prev => ({
            ...prev,
            layers: prev.layers.map(l => l.id === activeLayer.id ? { ...l, ...updates } : l)
        }));
    };
    ...
    const addNewLayer = () => {
        const newLayer: DrawingLayer = {
            ...
            visible: true,
            bindPlanetId: settings.bindPlanetId,
            tilt: { x: 0, y: 0, z: 0 },
            scale: 1,
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm font-semibold text-gray-300">
                        <div className="flex items-center gap-2">
                            ...
                            <span>组件图层 ({settings.layers?.length || 0})</span>
                        </div>
                        <button
                            onClick={addNewLayer}
                    ...
                    <div className="bg-black/30 rounded-lg p-2 max-h-40 overflow-y-auto space-y-1 border border-white/5">
                        {settings.layers?.map(layer => (
                            <div
                                key={layer.id}
                                onClick={() => setSettings(prev => ({ ...prev, activeLayerId: layer.id }))}
                                className={`flex items-center justify-between px-2 py-1.5 rounded cursor-pointer group transition-colors
                                    ${settings.activeLayerId === layer.id ? 'bg-purple-600/30 border border-purple-500/30' : 'hover:bg-white/5 border border-transparent'}`}
                            >
                                <div className="flex items-center gap-2 overflow-hidden">
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            updateActiveLayer({ visible: !layer.visible }); // Need to target specific layer, not active
                                            setSettings(prev => ({
                                                ...prev,
                                    ...
                                    <span className={`text-xs truncate ${settings.activeLayerId === layer.id ? 'text-purple-200 font-medium' : 'text-gray-400'}`}>
                                        {layer.name}
                                    </span>
                                </div>
                                <button
                ...
                {activeLayer && (
                    <div className="space-y-3 p-3 bg-purple-900/10 rounded-xl border border-purple-500/10">
                        ...
                        <div className="flex items-center justify-between text-xs mb-2">
                            <span className="text-gray-400">归属星球</span>
                            <select
                                value={activeLayer.bindPlanetId || ''}
                                onChange={(e) => updateActiveLayer({ bindPlanetId: e.target.value })}
                                className="bg-black/40 border border-white/10 rounded px-2 py-1 text-gray-200 outline-none focus:border-purple-500"
                            >
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            updateActiveLayer({ altitude: parseFloat(e.target.value) });
                                            setSettings(prev => ({ ...prev, currentAltitude: parseFloat(e.target.value) }));
                                        }}
                                        className="w-full h-1 bg-gray-700 rounded appearance-none"
                                    />
                                </div>
