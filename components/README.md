...
- ControlPanel.tsx
  ...
  - 功能: 修改 settings、管理星云实例、预设/导入导出；星球模式视觉效果（Bloom 完整参数控制）；星球模式 5 类旋转速度范围：能量核-6~6、粒子环-8~8、连续环-10~10、粒子环绕-6~6、实体核心-10~10
...
- PlanetScene.tsx
  - 地位: 星球与互通模式渲染核心
  - 功能: 渲染星球场景；互通模式下接管星云实例渲染并同步 uniforms；运行时同步自定义法阵各笔刷材质 uniforms（包含 lightsaber 的 uMC* 染色/饱和度命名）；确保自定义法阵各笔刷(Particle/Lightsaber/Line)均正确接收symmetryParams以支持分形渲染；iPad 端自定义法阵丝环/光剑开启自转时，需保留笔刷内部细粒度 renderOrder 并按 layerIndex 叠加大偏移以稳定透明排序（避免闪烁）；实现 Bloom 后处理参数的实时更新
...
- AIAssistantPanel.tsx
  - 地位: AI 交互面板主入口
  - 功能: 灵感模式图片生成、消息渲染、XingSpark 设置集成、模型选择；创造模式代码执行与管理；后台调用代码分析 API 提取可编辑参数；管理选中效果并协调参数更新

- AIEffectParamsPanel.tsx
  - 地位: AI 效果参数编辑器（左侧面板）
  - 功能: 渲染从 AI 代码中提取的可编辑参数（数值/颜色/开关）；提供参数实时修改与重置功能；仅在选中效果时显示
