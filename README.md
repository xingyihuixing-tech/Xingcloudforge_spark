...
- 粒子画笔密度改为按笔迹弧长生成（避免因 pointer 采样点稀疏导致“加密度仍稀疏”）。
- 支持压感模式：`无` / `书法` / `亮度`，并随 stroke 参数持久化。
- 粒子密度 UI 范围：`100-800`（按弧长密度）。

## 自定义法阵绘图：丝环画笔

- 支持压感模式：`无` / `书法` / `亮度`；`书法/亮度` 模式沿笔画分段生效，并随 stroke 参数持久化。
- iPad 端开启图层自转时，为避免透明 Additive 材质排序抖动导致闪烁，渲染侧会保留笔刷内部细粒度 `renderOrder`，并按 `layerIndex` 叠加大偏移进行分层排序。

## 自定义法阵绘图：光剑画笔

- 光剑画笔使用独立 shader 命名（`uMC*`），其染色/饱和度等法阵级参数由 `PlanetScene` 在运行时同步到材质 uniforms。
- iPad 端开启图层自转时，为避免透明 Additive 材质排序抖动导致闪烁，渲染侧会保留笔刷内部细粒度 `renderOrder`，并按 `layerIndex` 叠加大偏移进行分层排序。
